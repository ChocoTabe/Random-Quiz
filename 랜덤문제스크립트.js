  ...
  $( function() {
    ...
    $( '#quizStart' )
      .click( function() {
        ...
        $( '#image' )
          .on( 'mousedown mousemove touchstart', function( event ) {
        ...
        function nextFunc() {
          ...
          document.getElementById( 'youtube' )
            .setAttribute( 'src', "" );
          document.getElementById( 'secYoutube' )
            .setAttribute( 'src', "" );
          document.getElementById( 'firstListen' )
            .setAttribute( 'src', "" );
          document.getElementById( 'secondListen' )
            .setAttribute( 'src', "" );
          document.getElementById( 'thirdListen' )
            .setAttribute( 'src', "" );
          $( '#answerChoice div, #subjective, #oxChoice div' )
            .slideDown();
          $( '#firstListen, #secondListen, #thirdListen, #image' )
            .slideUp();
          $( '#play2, #play3' )
            .hide();
          $( '#play1' )
        ...
        Loading();
        setTimeout( closeLoading, 2000 );
        $( '#title, #subTitle, #quizStart, #instruction, #how, #firstChoice' )
          .remove();
        $( '#fieldArea, #question, #exampleCell, #answerChoice, #grid div' )
          .show();
        $( '#grid div' )
        ...
        function algorism() {
          ...
          $( '#answer' )
            .click( function() {
              if ( firstList.objAnswer()
                .text() != false && firstList.subjAnswer()
                .text() == false && firstList.oxQAnswer()
                .text() == false ) {
                $( '#a' )
                  .show();
                $( '#a' )
                  .text( "정답은 : " + firstList.objAnswer()
                    .text() );
              } else if ( firstList.objAnswer()
                .text() == false && firstList.subjAnswer()
                .text() != false && firstList.oxQAnswer()
                .text() == false ) {
                $( '#b' )
                  .show();
                $( '#b' )
                  .text( "정답은 : " + firstList.subjAnswer()
                    .text() );
              } else {
                $( '#c' )
                  .show();
                $( '#c' )
                  .text( "정답은 : " + firstList.oxQAnswer()
                    .text() );
              }
            } );
          firstList.problem()
          ...
          firstList.example()
            .text();
          firstList.objAnsList()
            .text();
          firstList.objAnsList2()
            .text();
          firstList.objAnsList3()
            .text();
          firstList.objAnsList4()
            .text();
          firstList.objAnsList5()
            .text();
          firstList.oxQAnswer()
            .text();
          firstList.subjAnswer()
            .text();
          firstList.objAnswer()
            .text();
          $( '#answerList' )
          ...
          firstList.music()
            .text( function() {
              $( '#play2' )
                .click( function() {
                  var secFile = $( 'span.empty' )
                    .text();
                  secFile = secFile.substr( 19, 17 );
                  document.getElementById( 'secondListen' )
                    .setAttribute( 'src', secFile );
                } );
            } );
          firstList.music()
            .text( function() {
              $( '#play3' )
                .click( function() {
                  var thirdFile = $( 'span.empty' )
                    .text();
                  thirdFile = thirdFile.substr( 38, 52 );
                  document.getElementById( 'thirdListen' )
                    .setAttribute( 'src', thirdFile );
                } );
            } );
          firstList.image()
            .text( function() {
              $( '#showImage' )
                .click( function() {
                  var importImg = $( 'span.empty2' )
                    .text();
                  importImg = importImg.substr( 0, 17 );
                  var altSet = $( 'span.empty2' )
                    .text();
                  altSet = altSet.substr( 8, 9 )
                  document.getElementById( 'image' )
                    .setAttribute( 'src', importImg );
                  document.getElementById( 'image' )
                    .setAttribute( 'alt', altSet );
                } )
            } )
          firstList.video()
            .text( function() {
          ...
          if ( list == false ) {
            $( 'next' )
              .click( function() {
                $( '#fieldArea, #field, #question, #exampleCell, #example, #youtube, #secYoutube, #storageFrame, #storage, #a, #b, #c, #wrong, #answerChoice div, #answerChoice div span, #subjective, #oxChoice, #oxChoice #o, #oxChoice #X, #correct, #answers, #answers div, #answers div span, #grid, #grid div, #message, #play1, #play2, #play3, #showImage, #image, #firstList, #secondList, #thirdList' )
                  .hide()
                  .remove();
                $( '#consequenceA div' )
                  .slideDown( function() {
                    $( '#scoreA' )
                      .text( '당신이 맞춘 문제 : ' + correctThings.length );
                    $( '#scoreB' )
                      .text( '당신이 틀린 문제 : ' + wrongThings.length );
                    $( '#sum' )
                      .text( '지금까지 푼 문제 : ' + allThings.length );
                    $( '#complete' )
                      .show();
                  } );
              } )
        ...
        $( '#next' )
          .click( function() {
            if ( $( '#storage' )
              ...
              .text() == $( '#storage' )
              .text() ) {
              allThings.push( $( '#storage' )
                .text() );
              correctThings.push( $( '#storage' )
                .text() );
              alert( $( '#correct' )
                .text() );
              nextFunc();
        ...
        $( '#consequence' )
          .click( function() {
            $( '#fieldArea, #field, #question, #exampleCell, #example, #youtube, #secYoutube, #storageFrame, #storage, #a, #b, #c, #wrong, #answerChoice div, #answerChoice div span, #subjective, #oxChoice, #oxChoice #o, #oxChoice #X, #correct, #answers, #answers div, #answers div span, #grid, #grid div, #message, #play1, #play2, #play3, #showImage, #image, #audioGroup audio' )
              .hide()
              .remove();
            $( '#consequenceA div' )
              .slideDown( function() {
